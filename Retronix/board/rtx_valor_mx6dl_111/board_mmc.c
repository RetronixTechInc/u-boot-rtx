...
int board_mmc_get_env_dev(int devno)
{
	...
	//printf("trans mmc num: %d -> %d\n",devno ,iDevNoTable[devno]);
...
int board_mmc_getcd(struct mmc *mmc)
{
	...
	switch (cfg->esdhc_base) 
	{
		case USDHC1_BASE_ADDR:
			ret = !gpio_get_value(BOARD_PAD_SD1_CD);
			break;
...
int board_mmc_init(bd_t *bis)
{
	...
	gpio_direction_input(BOARD_PAD_SD1_CD);
	gpio_direction_input(BOARD_PAD_SD2_CD);
