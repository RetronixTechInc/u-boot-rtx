	...
	#define CONFIG_EXTRA_ENV_USE_DTB
		...
		#define CONFIG_EXTRA_ENV_SETTINGS \
			"hdmi=setenv bootargs ${bootargs} video=mxcfb2:dev=hdmi,1920x1080M@60,if=RGB24,bpp=32\0" \
			"set_mem=setenv bootargs ${bootargs}\0" \
			"fecmac_val=fec_mac=fa:3a:65:c7:14:eb\0" \
			"set_display=run hdmi\0" \
			"bootargs_base=setenv bootargs console=ttymxc1,115200 ${fecmac_val}\0"\
			"bootargs_gen=setenv bootargs ${bootargs} ip=off ${root_loc} rootfstype=ext4 rootwait rw\0"\
			"r_kernel=mmc read ${loadaddr} 0x6900 0x4000\0" \
			"r_ramdisk=mmc read ${rd_loadaddr} 0x3000 0x3000\0" \
			"r_dtb=mmc read ${dtb_loadaddr} 0x6800 0x100\0" \
			"storage=mmc dev 2\0" \
	...
	#define CONFIG_SYS_PROMPT		                "RTX-PITX MX6D U-Boot > "
